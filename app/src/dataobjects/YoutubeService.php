...
/**
	...
	function extractVideoInfo($video) {
		...
		$data['PlayerURL'] = str_replace('/v/', '/watch?v=', $data['PlayerURL']);
		//$test = $mediaentry->xpath('media:player');
		//debug::show($data['PlayerURL']);
		//debug::show(Convert::raw2xml((string)$test[0]['url']));	// this is a simpler way to get the URL - the above works for now though
